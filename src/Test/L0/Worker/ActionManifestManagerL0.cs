    ...
    public sealed class ActionManifestManagerL0
    {
        ...
        public void Load_CompositeActionNoUsing()
        {
            try
            {
                ...
                _ec.Verify(x => x.AddIssue(It.Is<Issue>(s => s.Message.Contains("Missing 'using' value. 'using' requires 'composite', 'docker', 'node12', 'node16' or 'node18'.")), It.IsAny<string>()), Times.Once);
            }
            finally
            {
                Teardown();
