    ...
    public sealed class ActionManifestManager : RunnerService, IActionManifestManager
    {
            ...
            MappingToken outputs = null)
        {
            ...
            if (usingToken != null)
            {
                ...
                {
                    throw new ArgumentOutOfRangeException($"'using: {usingToken.Value}' is not supported, use 'docker', 'node12', 'node16' or 'node18' instead.");
                }
            }
            ...
            throw new NotSupportedException("Missing 'using' value. 'using' requires 'composite', 'docker', 'node12', 'node16' or 'node18'.");
        }
